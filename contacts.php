...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($nameErr) && empty($emailErr) && empty($messageErr)) {
    ...
    $conn = pg_connect("host=localhost dbname=ecotrack user=postgres password=admin");

    if (!$conn) {
      $successMsg = "Database connection failed: " . pg_last_error();
    } else {
      ...
      if ($result) {
        ...
        $successMsg = "Failed to save message: " . pg_last_error($conn);
      }
