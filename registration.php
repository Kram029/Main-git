...
// Load city
$stmt = $conn->prepare("CALL sp_get_municipalities()");
$stmt->execute();
$res = $stmt->get_result();
...
// Load barangay
if ($city) {
  $stmt = $conn->prepare("CALL sp_get_barangays(?)");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!$xfullname && !$xemail && !$xcontact && !$xstreet && !$xusername && !$xpassword && !$xconfirm) {
    ...
    $stmt = $conn->prepare("INSERT INTO table_users_registration (first_name, middle_name, last_name, suffix, email, contact, city_id, barangay_id, street, username, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param(
      "ssssssissss",
      $fullname['first_name'],
...
<div class="form-container">
  <div class="form-box">
    ...
    <form method="POST" class="needs-validation" novalidate>
      ...
      <div class="mb-3">
        <label for="city" class="form-label">Current Address</label>
